...
// Extract 4-digit year from various formats like "7 ян. 2013"
...
function getSortWord(entry) {
  ...
  // Multiple authors + NOT GOI → first author inverted + "и др."
  if (parts.length > 1 && !itemTypes.includes("GOI")) {
    const first = normalizeAuthorName(parts[0]);
    let inverted = first;
    if (!first.includes(",")) {
...
function formatArticle(entry) {
  ...
  if (responsibility && !itemTypes.includes("GOI")) line2Parts.push({ text: ` / ${responsibility}` });
  if (source) {
    line2Parts.push({ text: `. – В: ` });
    line2Parts.push({ text: source, italics: true });
  }
...
function formatYearbook(entry) {
  ...
  const editionRaw = getFirst(entry, "edition") || "";
  const publisher = getFirst(entry, "publisher") || "";
  ...
  if (editionRaw) {
    if (/с\./i.test(editionRaw)) editionExtent = editionRaw;
    else editionPub = editionRaw;
  }
  ...
  if (editionPub) isbdLine += ". – " + editionPub;
  ...
  if (finalExtent) isbdLine += ". – " + finalExtent;

  if (isbdLine) lines.push(isbdLine);
  if (aboutPersons.length > 0) lines.push("Имена на лица, за които става дума: " + aboutPersons.join(", "));
...
const parsed = entries.map((entry) => {
  ...
  const rawYear = getFirst(entry, "year");
...
function pushEntry(formatFn, rawEntries) {
  rawEntries.forEach((entry) => {
    ...
    if (mainLine) {
      mainLine.split("\n").forEach((line) => {
        if (line.trim()) children.push(new Paragraph({ children: [new TextRun({ text: line, size: 24 })] }));
      });
    }
      ...
      children.push(
        new Paragraph({
          children: styledLine2.map(
            (part) => new TextRun({ text: part.text, italics: part.italics || false, bold: part.bold || false, size: 24 })
          ),
        })
      );
    }

    if (itemTypeLine) children.push(new Paragraph({ children: [new TextRun({ text: itemTypeLine, size: 24 })] }));
    ...
    if (otherSources && otherSources.trim()) {
      children.push(new Paragraph({ children: [new TextRun({ text: "\tВж. и: " + otherSources, size: 24 })] }));
    }
...
if (books.length > 0) {
  ...
  books.forEach((b) => {
    if (yearbooks.includes(b)) pushEntry(formatYearbook, [b.entry]);
    else pushEntry(formatBook, [b.entry]);
  });
}

if (articles.length > 0) {
  children.push(new Paragraph({ children: [new TextRun({ text: "СТАТИИ", bold: true, size: 24 })] }));
  pushEntry(formatArticle, articles.map((a) => a.entry));
}

if (others.length > 0) {
  children.push(new Paragraph({ children: [new TextRun({ text: "ДРУГИ", bold: true, size: 24 })] }));
  pushEntry(formatOther, others.map((o) => o.entry));
}
